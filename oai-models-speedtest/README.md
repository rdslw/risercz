...
2. Select 1–20 tries per model/mode, a total budget, an approximate generated input size (10K–100K tokens), and an output cap (2K–20K tokens). The page does not expose an editable prompt: it generates repeated one-thousand-token lorem-ipsum blocks and instructs the model to write an original field guide up to the requested output cap.
