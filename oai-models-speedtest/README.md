...
The page includes a restrictive CSP before its inline script: it permits connections only to `https://api.openai.com`, disables plugin objects, form submissions, and base-URL changes, and authorizes the one embedded script with a SHA-256 hash. This is not circular: the hash is calculated over the exact contents of the `<script>` element, not the HTML document or the CSP meta element that contains the hash. If the script changes, regenerate the hash before publishing; otherwise browsers will correctly block the changed script. Inline styles remain allowed because this self-contained page includes inline CSS and writes bounded bar widths into result elements.
